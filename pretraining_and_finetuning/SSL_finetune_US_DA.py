...
#pretrained_path = os.path.normpath("logdir/pretrain/best_model_US_DA.pt")
...
train_transforms = Compose(
    [
        ...
        Rand3DElasticD(
            keys=["image", "label"], prob=0.30, sigma_range=(0.1, 0.2), magnitude_range=(0.1, 0.2)
        ),
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        if (global_step % eval_num == 0 and global_step != 0) or global_step == max_iterations:
            ...
            if dice_val > dice_val_best:
                ...
                torch.save(model.state_dict(), os.path.join(logdir, "best_metric_model_without_pretraining_US_DA1811.pth"))
                print(
            ...
            plt.plot(x, y)
            plt.grid()
            plt.savefig(os.path.join(logdir, "finetune_quick_update_US_DA_notpretrained1811.png"))
            plt.clf()
            plt.close(1)
...
model.load_state_dict(torch.load(os.path.join(logdir, "best_metric_model_without_pretraining_US_DA1811.pth")))
model.eval()
...
with torch.no_grad():
    for i, val_data in enumerate(val_loader):
        ...
        plt.subplot(1, 3, 1)
        plt.title(f"image {i}")
        plt.imshow(val_data["image"][0, 0, :, :, 150], cmap="gray")
        plt.subplot(1, 3, 2)
        plt.title(f"label {i}")
        ...
        plt.savefig("Validation_results_US_DA_notpretrained1811.png")
        plt.show()
        if i == 3:
            break
