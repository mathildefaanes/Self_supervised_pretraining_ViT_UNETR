...
json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), "datasets/dataset_splits_unlabeled.json")
print(json_path)
data_root = os.path.normpath("../datasets/Unlabeled/")
logdir_path = os.path.normpath("logdir/pretrain")

# Create results logging directories, manage data paths and set determinism
    ...
    [
        ...
        OneOf(
            transforms=[
                ...
                Rand3DElasticD(
                    keys=["image"], prob=1.0, sigma_range=(0.1,0.2), magnitude_range=(0.1,0.2)
                ),
                RandRotateD(
                    keys=["image"], prob=1.0
                ),
        ...
        OneOf(
            transforms=[
                RandGaussianSmoothD(
                    keys=["image_2"], prob=1.0
                ),
                AdjustContrastD(
                    keys=["image_2"], gamma=1
                ),
                Rand3DElasticD(
                    keys=["image_2"], prob=1.0, sigma_range=(0.1,0.2), magnitude_range=(0.1,0.2)
                ),
                RandRotateD(
                    keys=["image_2"], prob=1.0
                ),
...
for epoch in range(max_epochs):
    ...
    if epoch % val_interval == 0:
        ...
        if total_val_loss < best_val_loss:
            ...
            torch.save(checkpoint, os.path.join(logdir_path, "best_model_US_DA.pt"))
        ...
        plt.savefig(os.path.join(logdir_path, "loss_plots_DA_US.png"))
        plt.close(1)
